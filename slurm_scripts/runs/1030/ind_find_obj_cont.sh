...
export EXP_NAME=find_obj/input_${INPUTS}_${ENVS}x${GPUS_PER_NODE}x${NODES}_envs_${EPS_KEY}_explore_reward_${EXPLORE_REWARD}_no_augs_${NO_AUGS}_navmesh_pen_${NAVMESH_PEN}_cont_actions_${CONT_ACTIONS}_must_call_stop_${CALL_STOP}_must_face_${MUST_FACE}



if [ $CONT_ACTIONS = true ]; then
    export MORE_OPTIONS="benchmark/ovmm=nav_to_obj"
    export MORE_OPTIONS="${MORE_OPTIONS} habitat/task/ovmm=nav_to_obj_cont"
else
    export MORE_OPTIONS="benchmark/ovmm=nav_to_obj"
fi
