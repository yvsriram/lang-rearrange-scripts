...
export EXP_NAME=multi_task/input_${INPUTS}_${ENVS}x${GPUS_PER_NODE}x${NODES}_envs_${EPS_KEY}_no_augs_${NO_AUGS}_navmesh_pen_${NAVMESH_PEN}_cont_actions_${CONT_ACTIONS}_find_obj_small_range
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.actions.base_velocity.min_displacement=0.24 habitat.task.actions.base_velocity.max_displacement_along_axis=0.25 habitat.task.actions.base_velocity.min_turn_degrees=29 habitat.task.actions.base_velocity.max_turn_degrees=30"
