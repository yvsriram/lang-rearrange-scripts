...
export EXP_NAME=end_to_end/input_${INPUTS}_${ENVS}x${GPUS_PER_NODE}x${NODES}_envs_${EPS_KEY}_no_augs_${NO_AUGS}_navmesh_pen_${NAVMESH_PEN}_cont_actions_${CONT_ACTIONS}_v2_max_${MAX_COLLS}_coll_pen_${COLL_PEN}_place
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.success_measure=ovmm_place_success habitat.task.place_init=True habitat.task.skills_completed_at_start=3 habitat.task.base_angle_noise=0.0873 habitat.task.start_in_manip_mode=True"
