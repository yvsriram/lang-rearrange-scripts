...
export SKILL_REWARD=5.0
export CALL_STOP=true
# export MUST_FACE=true
# export CALL_STOP=true
export EXP_NAME=end_to_end/input_${INPUTS}_${ENVS}x${GPUS_PER_NODE}x${NODES}_envs_${EPS_KEY}_no_augs_${NO_AUGS}_navmesh_pen_${NAVMESH_PEN}_cont_actions_${CONT_ACTIONS}_v2_max_${MAX_COLLS}_coll_pen_${COLL_PEN}_find_obj_pick_find_rec_only_nav_${ONLY_NAV}_call_stop_${CALL_STOP}_skill_reward_${SKILL_REWARD}
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.success_measure=ovmm_find_recep_phase_success habitat.task.skills_completed_at_start=0"
