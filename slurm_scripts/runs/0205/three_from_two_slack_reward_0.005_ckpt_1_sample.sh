...
export PRETRAINED_PATH="data/new_checkpoints/ovmm_new/input__16x1x8_envs_new_train_no_augs_false_navmesh_pen_0.0_cont_actions_true_v2_max_-1_coll_pen_0.0_mobile_place_false_call_stop_false_constraint_base_false_two_steps_ckpt_2_drop_shld_end_dropout_0.0_pretrained_warm_start/ckpt.1.pth"
export SPARSE_REWARD=true

export SLACK_REWARD=0.005
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.pick_init=True"

export ALLOWED_SKILLS="['gaze','find_rec','place']"

export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.skill_schedule=${SCHEDULE} habitat.task.allowed_skills=${ALLOWED_SKILLS}"

export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.success_measure=ovmm_place_success habitat.task.skills_completed_at_start=1 habitat.task.object_in_hand_sample_prob=0.0"
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.slack_reward=-${SLACK_REWARD}"
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat_baselines.trainer_name=ddppo habitat_baselines.total_num_steps=1.0e9 habitat_baselines.num_checkpoints=100"
...
export MORE_OPTIONS="${MORE_OPTIONS} habitat.environment.max_episode_steps=800"
