...
export INPUTS=obj_emb_rgb
export OBS_KEYS="['head_rgb','start_receptacle','start_recep_segmentation','object_segmentation','object_embedding','joint']"
...
export MORE_OPTIONS="benchmark/ovmm=gaze_rgb"
...
export EXP_NAME="${EXP_NAME}-success_dist_${gaze_distance}"_no_augs_${NO_AUGS}_train_long_fixed_robust_states_wrt_target

export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.spawn_reference=target habitat.task.spawn_max_dist_to_obj=2.0 habitat.task.base_angle_noise=3.14"

if [ $NO_AUGS = true ]; then
export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.lab_sensors.object_segmentation_sensor.blank_out_prob=0.0 habitat.task.lab_sensors.start_recep_segmentation_sensor.blank_out_prob=0.0"
fi
