...
export EXP_NAME=${SKILL}/input_${INPUTS}_${ENVS}x${GPUS_PER_NODE}x${NODES}_envs_${EPS_KEY}_no_augs_${NO_AUGS}_sparse_${SPARSE_REWARD}_max_surface_steps_${MAX_SURFACE_STEPS}_cons_base_manip_mode_${CONSTRAINT_BASE_IN_MANIP_MODE}_coll_pen_${COLLISIONS_PEN}_${COLLISIONS_END_PEN}_max_${MAX_COLLS}_colls_robust_2m
...
if [ $NO_AUGS = true ]; then
    export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.lab_sensors.goal_recep_segmentation.blank_out_prob=0.0"
else
    export MORE_OPTIONS="${MORE_OPTIONS} habitat.task.lab_sensors.goal_recep_segmentation.blank_out_prob=${DROPOUT}"
